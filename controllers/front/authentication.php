...
class TraxxAuthenticationModuleFrontController extends ModuleFrontController 
{
	...
	public function initContent()
	{
		...
		if ( strcasecmp( $_POST['auth_hash'], $hash_check ) != 0 ) {
			...
			$i = $_POST['cart_id'] . Configuration::get('PS_OS_TRAXX_AUTHORIZED') . $_POST['tran_amount'] . $this->module->displayName .  "999".  "999".  (int)$this->context->currency->id.  "999".  $_POST['xtra_secure_key'];

			$this->_logToFile(_LOG_DIR_.'debug.log', $i );
			// Hash check OK.  use details supplied in POST data to determine what action to take 
			// $POST['auth_status'] // A = Authorised, H = Authorised but on hold, any other character = not authorised

			//TODO: CREATE AN XTRA_FIELD FOR THE CUSTOMER SECURE KEY: CURRENTLY USING THE ADDRESS 3 FIELD TO HANDLE THE SECURE KEY
			$this->module->validateOrder($_POST['cart_id'], Configuration::get('PS_OS_TRAXX_AUTHORIZED'), $_POST['tran_amount'], $this->module->displayName, NULL, false, (int)$this->context->currency->id, false, $_POST['xtra_secure_key']);
		}
	}
